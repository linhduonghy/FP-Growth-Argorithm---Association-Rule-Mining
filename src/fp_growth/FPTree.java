...
public class FPTree {
	...
	public FPTree(Map<Transaction, Integer> transactions, double min_sup) {		
...
//		System.out.println(root);
	...
	private Map<Item, Integer> find_frequent_items(Map<Transaction, Integer> transactions, double min_sup) {
		...
		while (it.hasNext()) {
			...
			if ((double) entry.getValue()  < min_sup) {
				it.remove();
			}
		}
//		System.out.println(frequent_items);
	...
	private FPNode build_fptree(Map<Transaction, Integer> transactions,
			Map<Item, Integer> frequent, HeaderTable headers, double min_sup) {
